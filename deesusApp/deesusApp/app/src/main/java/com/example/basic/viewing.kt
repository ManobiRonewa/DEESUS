...
class viewing : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val recyclerView: RecyclerView = findViewById(R.id.recyclerViews)
    ...
    private inner class ProductAdapter(
        ...
        private fun addToCart(title: String, size: String, price: Double) {
            if (title.isNotBlank() && size.isNotBlank()) {
                ...
                Toast.makeText(this@viewing, "Invalid item details", Toast.LENGTH_SHORT).show()
            }
        }
...
// Create fields
            ...
            val userSession = user_active(this@viewing)
            ...
            confirmationBuilder.setPositiveButton("Confirm") { _, _ ->
                ...
                if (email.isNotEmpty()) {
                    lifecycleScope.launch {
                        ...
                        cartItems.clear()
                        sessionManager.saveCartItems(cartItems) // Update session
                        val intent = Intent(this@viewing,ShopActivity::class.java)
    ...
    fun backing(view: View) {
        val openProfileIntent = Intent(this, admin()::class.java)
        startActivity(openProfileIntent)
